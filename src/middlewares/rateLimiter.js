...
export const rateLimiterUsingThirdParty = rateLimit({
  windowMs: WINDOW_SIZE_IN_HOURS * 60 * 60 * 1000 * 30 , // 24 hrs in millseconds for 30 days
  max: MAX_WINDOW_REQUEST_COUNT,
  message: `You have exceeded the ${MAX_WINDOW_REQUEST_COUNT} requests on monthly limit!`,
  headers: true
});
    ...
    redisClient.get(req.ip, function(err, record) {
      ...
      if (totalWindowRequestsCount >= MAX_WINDOW_REQUEST_COUNT) {
        res
          .status(429)
          .jsend.error(
            `You have exceeded the ${MAX_WINDOW_REQUEST_COUNT} requests in 1 Month limit! To increase the limit Please contact X,Y,Z Company`
          );
      } else {
